...
const Categories = () => {
    ...
    const actionFormat = (id, cell) => {
        return <div className='d-flex justify-content-center'>
            <Popconfirm title="Voulez-vous vraiment supprimer cette catégorie ?" onConfirm={() =>
    ...
    const handleSubmit = async (event) => {
        ...
        if ('name' in response) {
            id ? message.success("Catégorie modifiée avec succès")
    ...
    async function category(method, data, id = '', ctype) {
        ...
        <div className='d-flex'>
            ...
            <Container className='p-0'>
                <Container className='bg-blue ps-5 d-flex align-items-center' style={{ height: 220 }}>
                    <h1 className='text-white p-0 m-0'><FontAwesomeIcon icon={Icons.faTable} className='me-3' /> Gestion des catégories</h1>
                </Container>
            ...
            <Modal show={show} onHide={handleClose} animation={true} centered>
                <Modal.Header closeButton>
                    <Modal.Title>{data['id'] ? 'Modifier' : 'Ajouter'} une catégorie</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Spin spinning={loading}>
